...
#cmake -DOPENCMISS_TOOLCHAIN=intel -DOPENCMISS_MPI=intel -DOPENCMISS_INSTRUMENTATION=vtune -DOPENCMISS_BUILD_TYPE=Debug .
...
# The units used here is. If you want to use other units you need to set it down.
...
#  L_UNIT = 1e3
#else :
#  # Please adjust the units down here
#  L_UNIT=1
...
## Problem parameters
#T_UNIT= T_UNIT 
#k_UNIT= E_UNIT/(L_UNIT*T_UNIT)
#rhoC_UNIT= E_UNIT*t_UNIT/(L_UNIT^3*T_UNIT)
#Source_UNIT= E_UNIT/L_UNIT^3
...
FileName_ele = "Mesh/final/cylinder.ele"
numberOfNodes    = 11476
...
FileName_node = "Mesh/final/correctedBoundaryMarker.node"
...
with open(FileName_node, "r") as f:
    ...
    for lineNum,line in enumerate(target):
        if lineNum !=0:
          ...
          x = float(target[lineNum][1]) * Units * 10  # times 10 to convert cm to mm. 
          y = float(target[lineNum][2]) * Units * 10
          z = float(target[lineNum][3]) * Units * 10
...
print( "number of chopped boundary nodes = %d"%len(choppedBoundary))
print(choppedBoundary)
...
equationsSetSpecification = [iron.EquationsSetClasses.CLASSICAL_FIELD,
        ...
        iron.EquationsSetSubtypes.CONSTANT_SOURCE_DIFFUSION] # For problem it was linear and for equations it was constant before
...
dependentField.ComponentValuesInitialise(iron.FieldVariableTypes.U,iron.FieldParameterSetTypes.VALUES,1,37.0)
...
#materialsField.ComponentValuesInitialise(iron.FieldVariableTypes.U,iron.FieldParameterSetTypes.VALUES,1,0.75/2.3e6*1e6) 
#materialsField.ComponentValuesInitialise(iron.FieldVariableTypes.U,iron.FieldParameterSetTypes.VALUES,2,0.75/2.3e6*1e6) 
#materialsField.ComponentValuesInitialise(iron.FieldVariableTypes.U,iron.FieldParameterSetTypes.VALUES,3,0.75/2.3e6*1e6) 


#materialsField.ComponentValuesInitialise(iron.FieldVariableTypes.U,iron.FieldParameterSetTypes.VALUES,4,1)
...
#      materialsField.ParameterSetUpdateElement(iron.FieldVariableTypes.U,iron.FieldParameterSetTypes.VALUES, 
#  elementNumber,4, 1.0)  #4.0e6*1e-9 0.42/4.0e6
...
#    elementDomain = decomposition.ElementDomainGet(elementNumber)
#    if elementDomain == computationalNodeNumber:
#      sourceField.ParameterSetUpdateElement(iron.FieldVariableTypes.U,iron.FieldParameterSetTypes.VALUES,elementNumber,1,0)
#for elementNumber in leftHumerusElements:
#    elementDomain = decomposition.ElementDomainGet(elementNumber)
#    if elementDomain == computationalNodeNumber:
#      sourceField.ParameterSetUpdateElement(iron.FieldVariableTypes.U,iron.FieldParameterSetTypes.VALUES,elementNumber,1,0)
...
#!!      & CMFE_BOUNDARY_CONDITION_FIXED,100.0_CMISSRP,Err)
#!  CALL cmfe_BoundaryConditions_SetNode(BoundaryConditions,DependentField,CMFE_FIELD_U_VARIABLE_TYPE,1,1,9,1, &
#!      & CMFE_BOUNDARY_CONDITION_NEUMANN_INTEGRATED,16.0_CMISSRP,Err)VALUES(1)
...
#!      & CMFE_BOUNDARY_CONDITION_FIXED,50.0_CMISSRP,Err)            
#!  CALL cmfe_BoundaryConditions_SetNode(BoundaryConditions,DependentField,CMFE_FIELD_U_VARIABVALUES(1)LE_TYPE,1,1,154,1, &
#!      & CMFE_BOUNDARY_CONDITION_FIXED,50.0_CMISSRP,Err)  
...
#boundaryConditions.SetNode(dependentField,ir on.FieldVariableTypes.DELUDELN,1,1,6,1,iron.BoundaryConditionsTypes.ROBIN,[110.0,120.0,110.0])
nodes = iron.Nodes()
region.NodesGet(nodes)
...
for nodeNumber in boundary:
  nodeDomain = decomposition.NodeDomainGet(nodeNumber,1)
  if nodeDomain == computationalNodeNumber:
#    boundaryConditions.SetNode(dependentField,iron.FieldVariableTypes.U,1,1,nodeNumber,1,iron.BoundaryConditionsTypes.FIXED,[10.0])
#    boundaryConditions.SetNode(dependentField,iron.FieldVariableTypes.DELUDELN,1,1,nodeNumber,1,
#      iron.BoundaryConditionsTypes.NEUMANN_POINT,[-1.0*0.42*kFactor/(4.0e6*rhoCFactor)])

    boundaryConditions.SetNode(dependentField,iron.FieldVariableTypes.DELUDELN,1,1,nodeNumber,1,
      iron.BoundaryConditionsTypes.ROBIN,[(1.68*hUnit)/(4.0e6*rhoCFactor),(16.8*q_hUnit)/(4.0e6*rhoCFactor)]) 
#    boundaryConditions.SetNode(dependentField,iron.FieldVariableTypes.DELUDELN,1,1,nodeNumber,1,
#      iron.BoundaryConditionsTypes.ROBIN,[0.0,-1.0*0.42*kFactor/(4.0e6*rhoCFactor)]) 
